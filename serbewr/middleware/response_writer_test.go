...
// mockHijackableResponseWriter implements http.ResponseWriter and
// http.Hijacker for testing.
type mockHijackableResponseWriter struct {
	*httptest.ResponseRecorder
}

func (m *mockHijackableResponseWriter) Hijack() (
...
func TestBaseResponseWriter_Hijack(t *testing.T) {
	tests := []struct {
		...
		{
			name: "hijacker fails",
			responseWriter: &mockFailingHijacker{
				ResponseRecorder: httptest.NewRecorder(),
			},
			expectError: true,
		},
